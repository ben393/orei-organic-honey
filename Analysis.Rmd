---
title: "Pesticide contamination in honey from organically managed colonies in the continental USA is comparable to certified organic imports"
subtitle: "Data and analysis"
author: 
 - Benedict X. DeMoras^[Cornell University, benedict.demoras@gmail.com]
...
library(ggspatial)      # Provides legend scale and north for map
...
Analysis, results and figures are organized according to where they appear in the manuscript.
...
```{r mapAPI, include=FALSE}
# Obtain your own Static Tiles API key from Mapbox.com first.
# basemaps::set_defaults(map_service = "mapbox", map_token = "<YOUR API KEY HERE>")

# Retrieve my API key
source("functions/basemaps_set_defaults.R")
```
...
### 3.1. Pesticide contamination across sampled honeys
...
### 3.2. Pesticide contamination relative to USDA organic tolerances

#### Stats for figure 5: Difference in likelihood of samples exceeding MRLs

```{r stats--fig5A}
# Prepare data ----------------------------------------------------------------
df.fig5A.MRL <- df.fig2.heat %>%
  group_by(Sample.ID, Group) %>%
  summarize(
    # The binomial model expects Yes and No data as 1 or 0
    SampleExceed.model = ifelse(any(Exceed.Tol == "Yes"), 1, 0),
    # But for plotting, we want Yes and No labels
    SampleExceed.plot = ifelse(any(Exceed.Tol == "Yes"), "Yes", "No"),
    .groups = "drop"
  )

df.fig5B.MRLHeat <- df.fig2.heat %>%
  group_by(Cmpd) %>%
  filter(any(Exceed.Tol == "Yes")) %>%
  mutate(Cmpd = case_match(Cmpd, "Piperonyl butoxide" ~ "PBO", .default = Cmpd))

# Build models --------------------------------------------------------------
df.modFig5A.prior <- data.frame(
  prior = "normal(0, 2.5)",
  class = "fixef"
)

m.fig5A <- glmmTMB(
  SampleExceed.model ~ Group,
  data   = df.fig5A.MRL,
  family = "binomial",
  priors = df.modFig5A.prior
  )

m.fig5ANULL <- glmmTMB(
  SampleExceed.model ~ 1,
  data   = df.fig5A.MRL,
  family = "binomial",
  priors = df.modFig5A.prior
  )

anova(m.fig5A, m.fig5ANULL)

emmeans(m.fig5A, pairwise ~ Group, type = "response")
```

#### Figure 5. Samples exceeding surveillance MRLs for organic honey.

```{r fig5, fig.height=5, fig.width=7}
# Fig 5A: Plot w/ proportion of samples above MRL ---------------------------------
fig.5A <- ggplot(
  data = df.fig5A.MRL,
  aes(x = Group, fill = SampleExceed.plot)
) +
  geom_bar(position = "fill") +
  geom_jitter(
    aes(y = ifelse(SampleExceed.plot == "Yes", 1, 0)),
    position = position_jitter(height = 0.01, width = 0.3),
    shape = 1,
    alpha = 0.6,
    color = "black"
  ) +
  # Wrap the Group labels at 13 characters
  scale_x_discrete(labels = str_wrap(styles.group, width = 13)) +
  scale_fill_manual(
    name = "Exceeds organic\nMRLs?",
    values = colors.tols,
    breaks = c("Yes", "No")
  ) +
  ylab("\nProp. of samples >MRL\n") +
...
fig.5B <- ggplot(df.fig5B.MRLHeat) +
...
(fig.5A.abs <- df.fig5A.MRL %>%
  mutate(SampleExceed.plot = case_match(SampleExceed.plot, "Yes" ~ 1, "No" ~ 0)) %>%
  group_by(Group) %>%
  summarize(ProbOverMRL = mean(SampleExceed.plot)) %>%

  ggplot(aes(x = Group, y = ProbOverMRL, color = Group, fill = Group)) +
  ...
  scale_fill_manual(values = styles.colors) +
  scale_color_manual(values = styles.colors) +
...
```{r fig--S2, fig.height=6, fig.width=6, warning=FALSE}
