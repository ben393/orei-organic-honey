...
First, I import some basic metadata on the honey samples, pesticides, USDA organic tolerances, and experimental colonies.
...
First, I use the number of compounds per sample as the response variable.
...
emmeans(m.fig3A, pairwise ~ Group, type = "response") # back-transforms means
```

Then I look at the total amount of residues in each sample as the response variable.
...
emmeans(m.fig3B, pairwise ~ Group, type = "response") # back-transforms means
```
...
(fig.3 <- ((ggplot(
  data = df.fig3.summ,
  aes(x = Group, y = num.Cmpd, fill = Group)
) +
  styles.ylab.num +
  geom_text(
    data = df.fig3.signif,
  ...
  (ggplot(
    data = df.fig3.summ,
    aes(x = Group, y = ppb.sum, fill = Group)
  ) +
    styles.ylab.sumPPB +
    geom_text(
      data = df.fig3.signif,
