...
```{r fig1, fig.height=5, fig.width=4, message=FALSE, warning=FALSE}
# A dataframe of apiaries and GPS coordinates.
df.coords <- read.csv("data/landscape/apiary-locations-OREI.csv")

plot_map <- function(df_coords, res, expand, fPull) {
  # Convert list of apiaries into coordinates to plot
  map.coords <- df_coords %>% 
    # mutate(Farm = str_replace(Apiary, " [1-9]", "")) %>% # If we want to use names
    group_by(Farm.Redact) %>% 
      summarise(across(c(Lat, Lon), ~mean(.)), .groups = "drop") %>% 
  ...
  map <- ggplot(data = map.coords) +
    ...
    geom_sf(aes(geometry = geometry), pch = 21, fill = "black", color = "white", stroke = 1, size = 2) +
...
(fig.1 <- plot_map(df_coords  = df.coords, res = 0.4, expand = 2) + 
  geom_text_repel(aes(geometry = geometry, label = Farm.Redact),
              stat         = "sf_coordinates",
              box.padding  = 0.5, point.padding = 0.3, 
              max.overlaps = 13, max.time = 1,
              seed         = 42,
              force_pull   = 200,
...
(fig.3 <- ((ggplot(
  data = df.plots, aes(x = Group, y = num.Cmpd, fill=Group)) +
  styles.ylab.num + 
  geom_text(data = p.bar.signif,
            aes(x = Group, y = y.Nr, label = Contrast.Nr))) /
  
(ggplot(
  data = df.plots, aes(x = Group, y = ppb.sum, fill=Group)) +
  styles.ylab.sumPPB + 
  geom_text(data = p.bar.signif,
...
df.heat %>% 
  ...
  gt(rowname_col = "Cmpd") %>% 
    ...
    cols_label(
      Class.Pest = "Class",       n.detects   = "n",      pct.detect = "%",
      mean       = "Mean",        se          = "S.E.",   max        = "Max",
      Tol.OG.ppb = "Organic MRL", n.exceedTol = "n > MRL") %>% 
