...
sf.farms <- read.csv("data/landscape/apiary-locations-OREI.csv") %>% 
...
ISA.tab <- ISA$sign %>% 
  ...
  mutate(across(starts_with("s."), ~ ifelse(. == 1, gsub("s.", "", cur_column()), NA))) %>%
  ...
  rowwise() %>% 
  mutate(A = as.data.frame(ISA$A)[Cmpd,Groups],
...
if(!file.exists(MDS.path)) { # If the NMDS result does NOT exist, make it!
  set.seed(42)               # Ensures reproducibility
  8 <- metaMDS(comm = df.fig4.NMDS,
                 autotransform = F, distance = "bray", engine = "monoMDS",
                 k = 2, model = "global", maxit = 999, trymax = 500,
                 wascores = T, expand = T, noshare=F)
  saveRDS(MDS, file = MDS.path)
} else { # The file exists, read data from it.
...
# WITH Indicator compounds
(fig.4 <- plot_NMDS(MDS = MDS, grp = ISA.grp, indic = ISA.indic, 
                    labs = styles.group, colors = lighten(styles.colors, 0.2))[[1]])

ggsave("Figure_4.png", plot = fig.4, path = "figures",
       dpi = 500, width = 7, height = 5)
```
...
pairwise.adonis(df.fig4.NMDS, factors=ISA.grp) %>% 
  ...
  cols_label(
    pairs = "Pairs", F.Model = "F model", R2 = html("R<sup>2</sup>"),
