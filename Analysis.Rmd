...
df.mapLines <- st_sf(
  crs = st_crs(df.coordLab),
  Farm.Redact = df.coordLab$Farm.Redact,
  geometry    = st_sfc(mapply(function(p1, p2) {
                  st_cast(st_union(p1, p2), "LINESTRING")
                }, df.coordLab$geometry, df.coords$geometry, SIMPLIFY = FALSE)))

# Plot the map
(fig.1 <- plot_map(df_coords  = df.coords, res = 0.4, expand = 2) + 
    geom_sf(data = df.mapLines, linewidth = 0.5) +
    geom_text_repel(data = df.coordLab, aes(label = Farm.Redact, geometry = geometry),
                    stat = "sf_coordinates",
